...
def create_job_api(topic: str) -> dict:
...
def delete_job_api(job_id: int) -> dict:
...
with st.form("create_job_form", clear_on_submit=True):
    ...
    if submitted:
        if not topic.strip():
            ...
            try:
                result = create_job_api(topic.strip())
...
# ──────────────────────────────────────────────
# Job Detail (auto-refreshes every 5 seconds
# WITHOUT blinking the rest of the page)
# ──────────────────────────────────────────────
...
def show_job_detail():
    ...
    # Live progress message while processing
    ...
    if content_data:
        ...
        st.info("⏳ Content is being generated... This section updates automatically.")
